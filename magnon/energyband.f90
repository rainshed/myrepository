...
module Hamitonian
	...
	subroutine high_sym_line(k,knum)
		...
		!---TH--------
		k(1:knum/3,1) = vecn(0d0,0.5d0,knum/3)
		k(1:knum/3,2) = vecn(0d0,0.5d0,knum/3)
		k(1:knum/3,3) = vecn(0d0,-0.5d0,knum/3)
		!-------------

		!---HP-------
		k(knum/3+1:2*knum/3,1) = vecn(0.5d0,0.25d0,knum/3)
		k(knum/3+1:2*knum/3,2) = vecn(0.5d0,0.25d0,knum/3)
		k(knum/3+1:2*knum/3,3) = vecn(-0.5d0,0.25d0,knum/3)
		!------------

		!---PT-------
		k(2*knum/3+1:knum,1) = vecn(0.25d0,0d0,knum/3)
		k(2*knum/3+1:knum,2) = vecn(0.25d0,0d0,knum/3)
		k(2*knum/3+1:knum,3) = vecn(0.25d0,0d0,knum/3)
		!------------
	end subroutine
	!--------------------------------------------------------
	...
	function diag(H) result(eigen_E)
		...
		!----for zheev-----------
		integer,parameter :: N = 12, Lwork = 100
...
program main
	...
	do i = 1,knum
		E = diag(matmul(motion,Ham(k(i,1),k(i,2),k(i,3))))
