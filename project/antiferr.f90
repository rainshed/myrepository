...
!	integer,parameter :: dp = selected_real_kind(8)
	...
	real(8),parameter :: z=2       !coordination number
...
!		write(*,*) Eigenvalue
!		write(*,*) knum
!		write(20,*) knum,dble(Eigenvalue(1)),dble(Eigenvalue(2))
...
program main
	...
	use calc
	implicit none
	call calc_spin(-0.5d0,0.5d0,rSz1,rSz2)
	call iteration(0.5d0,-0.5d0,rSz1,rSz2)
	write(*,*) Sz1,Sz2
	!------------------------------------------------------------------------------------------------
!	real(dp),parameter :: k_inter=0.01,o_inter = 0.01 !k_inter is interval of k, o_inter is interval of omega
!	real(dp),parameter :: o_max = 3d0, o_min=0d0  !o_max is max value of omega at omega axial
...
!
!
!	complex(8) :: eigenvalue(2)
!	common eigenvalue
!	!------------------------------------------------------------------------------------------------------
!	k1 = vec(-k/2d0,k/2d0,k_inter)
!	omega = vec(o_min,o_max,o_inter)
!	open(unit=20,file='data/dispersion.dat')
...
!!			write(*,*) dble(eigenvalue(1)),dble(eigenvalue(2))
!		end do
!		write(20,*) k1(knum),dble(eigenvalue(1)),dble(eigenvalue(2))
!	end do
...
subroutine calc_spin(Sz1_init,Sz2_init,Sz1_new,Sz2_new)
	...
	k1 = vecn(-k/2d0+0.1,k/2d0-0.1,k_num)
	Phi = 0
	do l=1,order
		do m=1,order
			do knum=1,k_num
